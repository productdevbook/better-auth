...
async function createTestAdapter(
  ...
  // Store the config for later use (especially the adapterId)
  ...
  // Apply custom schema transformations from options if needed
  // This replaces the functionality previously handled by betterAuth
  const enhancedAdapter = {
    ...testAdapter,
    id: adapterConfig.adapterId, // Use the stored config instead of trying to access it through testAdapter
  }
...
describe('create Adapter Helper', async () => {
  ...
  describe('checking for the results of an adapter call, as well as the parameters passed into the adapter call', () => {
    describe('create', () => {
      it('should fill in the missing fields in the result', async () => {
        const res = await adapter.create({
          model: 'user',
          data: { name: 'test-name' },
        })
